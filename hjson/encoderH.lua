...
local function needsQuotes(s)
    local sequences = {
        ...
        "^,",
        "%s$",
        "[" .. (is53 and "\0" or "%z\1") .. "-\31" .. COMMONRANGE .. "]",
    }
...
local function needsEscapeML(s)
    local sequences = {
        "'''",
        "^[\\s]+$",
        "[" .. (is53 and "\0" or "%z\1") .. "-\8\11\12\14-\31" .. COMMONRANGE .. "]",
    }
    return containsSequences(s, sequences)
end
...
local function startsWithNumber(s)
    local integer = s:match("^[\t ]*(-?[1-9]%d*)") or s:match("^[\t ]*(-?0)")
    if integer then
        local frac = s:match("^(%.%d+)", #integer + 1) or ""
        local exp = s:match("^([eE][-+]?%d+)", #integer + #frac + 1) or ""
        local ending = s:match("^%s*$", #integer + #frac + #exp + 1)
            or s:match("^%s*[%[,%]}#].*$", #integer + #frac + #exp + 1)
            or s:match("^%s*//.*$", #integer + #frac + #exp + 1)
            or s:match("^%s*/%*.*$", #integer + #frac + #exp + 1)
            or ""
